...
export function OriginDossier({ origin, background, record, onChange }: { origin: string; background: string; record: OriginRecord; onChange: (record: OriginRecord) => void }) {
  ...
  return <section className="panel origin-dossier">
    <div className="panel-label">DOSSIÊ DE ORIGEM // V2</div>
    <div className="rule-pair"><article><span>FAMILIARIDADE · {origin}</span><p>{originRule.familiarity}</p></article><article><span>COBRANÇA</span><p>{originRule.charge}</p></article></div>
    <div className="rule-pair background"><article><span>BENEFÍCIO · {background}</span><p>{backgroundRule.benefit}</p></article><article><span>CUSTO DO ANTECEDENTE</span><p>{backgroundRule.charge}</p></article></div>
